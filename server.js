...
async function startServer() {
  ...
  async function getSmtpTransporter() {
    ...
    if (!pooledTransporter) {
      ...
      pooledTransporter = nodemailer.createTransport({
        ...
        secure: smtpPort === 465,
        pool: true,
        // Keep connections alive for high speed
        maxConnections: 5,
        maxMessages: 200,
        rateDelta: 1e3,
        rateLimit: 10,
    ...
    (async () => {
      try {
        ...
        const mailOptions = {
          ...
          to: adminEmail,
          subject: `\u{1F514} New Donation Alert: Rs. ${formattedAmount} from ${donation["Donor Name"] || "Contributor"}`,
          html: `
            <!DOCTYPE html>
            <html>
            <head>
              ...
              <div class="container">
                ...
                <div class="content">
                  ...
                  <table class="details-table">
                    ...
                    <tr><td class="label">Submission Date</td><td class="value">${donation.Date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0]}</td></tr>
        ...
        if (donorEmail && donorEmail.includes("@")) {
          const donorMailOptions = {
            ...
            to: donorEmail,
            subject: `\u{1F338} Donation Received (Pending Verification) - Rs. ${formattedAmount} \u2022 SWDO Portal`,
            html: `
              <!DOCTYPE html>
              <html>
              <head>
  ...
  app.post("/api/notify-donor-status", async (req, res) => {
    ...
    (async () => {
      try {
        ...
        const formattedAmount = Number(donation.Amount || 0).toLocaleString();
        const subject = isApproved ? `\u2705 Donation Receipt Verified: Rs. ${formattedAmount} - SWDO Relief Portal` : `\u26A0\uFE0F Donation Verification Status Update - Reference ${donation["Transaction ID"] || "Receipt"}`;
            ...
            <div class="container">
              ...
              <div class="content">
                ...
                <table class="table">
                  ...
                  <tr><td class="label">Date Verified</td><td class="value">${donation.Date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0]}</td></tr>
                </table>
  ...
  app.get("/api/test-email", async (req, res) => {
    const targetEmail = req.query.to || process.env.ADMIN_NOTIFICATION_EMAIL || "swdo.kpk@gmail.com";
    try {
      const result = await getSmtpTransporter();
      if (!result) {
