...
// 2. Add header
const oldHeader = '<th className="py-3 px-3 font-semibold text-right">Aid Amount</th>';
const newHeader = '{isAdmin && <th className="py-3 px-3 font-semibold text-center">Contact</th>}\\n                  <th className="py-3 px-3 font-semibold text-right">Aid Amount</th>';
code = code.replace(oldHeader, newHeader);

// 3. Increment colspan
const oldEmptyStateStr = '<td colSpan={isAdmin ? 9 : 8} className="text-center py-8 text-slate-400">';
const newEmptyStateStr = '<td colSpan={isAdmin ? 10 : 8} className="text-center py-8 text-slate-400">';
code = code.replace(oldEmptyStateStr, newEmptyStateStr);

// 4. Add body cell
const oldAmountCell = '<td className="py-3 px-3 text-right font-mono font-bold text-blue-600 dark:text-blue-400 whitespace-nowrap">';
const newAmountCell = `{isAdmin && (
...
code = code.replace(oldAmountCell, newAmountCell);

fs.writeFileSync('src/components/tabs/BeneficiariesTab.tsx', code, 'utf8');
console.log('Patched contact info in BeneficiariesTab table');
